...
export function generateReferralSummary(
    ...
    const summary = `🏥 *PATIENT REFERRAL - CareLinks*

📋 *Patient:* ${patientName || 'Not specified'}
📅 *Date:* ${timestamp}
⚠️ *Risk Level:* ${extraction.riskLevel.toUpperCase()}
...
${extraction.redFlags.length > 0 ? `🚨 *RED FLAGS:*
${extraction.redFlags.map((flag: string) => `• ${flag}`).join('\n')}
...
*RECOMMENDATION:*
${extraction.riskLevel === 'critical' ? '⚠️ URGENT: Immediate physician evaluation required' : extraction.riskLevel === 'high' ? 'Schedule appointment within 24 hours' : 'Routine follow-up as needed'}
